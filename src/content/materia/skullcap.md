---
name: "Skullcap"
latinName: "Scutellaria lateriflora (American) / S. baicalensis (Chinese)"
commonNames:
  - American skullcap
  - blue skullcap
  - mad dog skullcap
  - Chinese skullcap
  - Huang Qin
  - Baical skullcap
summary: "North American nervine with limited clinical evidence for sleep and mood. A 56-day trial found +1.22 hours sleep time. Mood improvement without sedation is the consistent signal. Rare but documented liver injury requires monitoring."

uses:
  - sleep
  - mood
  - anxiety
  - stress
  - nervine

traditional:
  traditions:
    - North American Eclectic medicine
    - Traditional Chinese Medicine
    - Contemporary Western herbalism
  convergent: false
  attributions:
    - source: "Indigenous North American / Eclectic Medicine (19th century)"
      text: "Primary American nervine. Used for nervous exhaustion, hysteria, chorea, delirium tremens, and insomnia. Described as 'one of the most reliable nervines in the materia medica' by Eclectic physicians. Name 'mad dog skullcap' reflects traditional use for calming excitable and agitated nervous states. Aerial parts prepared as tinctures and fluid extracts."
    - source: "Traditional Chinese Medicine (2,000+ years, S. baicalensis / Huang Qin)"
      text: "Documented in the Shennong Bencao Jing (~2nd century CE). Classified as a Heat-clearing, dampness-drying herb — fundamentally different from Western nervine use. Traditional indications: febrile diseases with lung heat (cough with yellow phlegm), damp-heat intestinal conditions, bleeding from heat, jaundice. Key classical formulas: Xiao Chai Hu Tang, Huang Lian Jie Du Tang. WHO monograph and European Pharmacopoeia inclusion. Not a nervine in TCM tradition."
    - source: "Contemporary UK/Irish herbal practitioners (Brock 2012, n=62 NIMH members)"
      text: "Still used primarily for anxiety, stress, muscle spasms, digestive complaints secondary to nervous tension, and stress-related hypertension. Highly regarded for safety and therapeutic index. Prescribed both standalone and in formula combinations — the 200-year Eclectic nervine tradition is unbroken in contemporary Western practice."

evidence:
  summary: "Limited but consistent positive signal for sleep and mood. The 2025 crossover RCT (n=66, 56 days) showed clinically meaningful sleep improvement with standardized S. lateriflora extract. Two smaller trials show mood benefit without sedation or cognitive impairment. Anxiolytic effects appear real but often don't reach significance on validated anxiety scales — the primary signal is global mood enhancement. Combination product studies (S. baicalensis, Dodd 2025; S. lateriflora + Cistus, Di Minno 2024) show additional signals but cannot isolate skullcap's contribution. Evidence base is thin — no systematic review of S. lateriflora RCTs exists, largest trial n=66."
  keyStudies:
    - title: "Di Minno et al. 2025 — Primary Insomnia RCT (S. lateriflora)"
      finding: "n=66 adults with mild-to-moderate primary insomnia; 400mg/day standardized extract (10% baicalin, BlueCALM), 56-day crossover. PSQI decreased 3.5 points (supplement) vs deteriorated in placebo group (p<0.001). Total sleep time increased 1.22 hours. Sleep efficiency +9%. The most methodologically robust RCT for S. lateriflora."
      year: 2025
    - title: "Brock et al. 2014 — Mood in Healthy Volunteers (S. lateriflora)"
      finding: "n=43 healthy adults; 1050mg/day (350mg × 3), 2-week crossover. Total Mood Disturbance significantly reduced (POMS, p<0.001) vs no change with placebo (p=0.072). Anxiety scores (BAI) not significant (p=0.191). Energy and cognition unaffected — mood improved without sedation or impairment."
      year: 2014
    - title: "Wolfson & Hoffmann 2003 — Anxiolytic Effects"
      finding: "Double-blind, placebo-controlled crossover in healthy volunteers. Noteworthy anxiolytic effects demonstrated. Earliest S. lateriflora RCT; full methodology not widely reported in secondary literature."
      year: 2003
    - title: "Dodd et al. 2025 — Stressed Adults RCT (S. baicalensis combination)"
      finding: "n=35 stressed adults; 15-day crossover. S. baicalensis + hawthorn + magnesium + chromium combination improved mood, reduced trait anxiety scores, reduced total mood disturbance, enhanced sleep time and quality. Combination product — S. baicalensis contribution cannot be isolated."
      year: 2025

preparations:
  - method: extract
    dosing: "400mg/day standardized to 10% baicalin (BlueCALM or equivalent)"
    notes: "Evidence-based for sleep. Matches the 2025 RCT protocol exactly. Most reliable if sleep is the primary goal."
  - method: capsule
    dosing: "350mg whole herb three times daily (1050mg/day total)"
    notes: "Validated for mood in the Brock 2014 trial. Species must be specified on label: S. lateriflora aerial parts."
  - method: tincture
    dosing: "2–4mL three times daily (1:5, 45% ethanol)"
    notes: "Traditional Western standard. Allows gradual dose adjustment. Mild, slightly bitter — more tolerable than valerian. Works well in small amount of water or juice."
    taste: "Mildly bitter, slightly grassy. Not harsh. Far more tolerable than valerian or ashwagandha."
  - method: tea
    dosing: "1–2g dried aerial parts per cup, 2–3 cups daily"
    notes: "Steep 10–15 minutes covered. Mildly bitter and grassy — one of the more pleasant medicinal herb teas. Combine with valerian or lemon balm. Standard dose is roughly 1 teaspoon of dried herb per cup."

trying:
  duration: "2 weeks minimum for mood effects (matches trial duration). 8 weeks (56 days) for sleep — this matches the evidence-based trial duration and clinical significance threshold."
  whatToNotice:
    - Sleep onset time and total sleep duration (from week 1)
    - Sleep quality on waking — refreshed vs. still tired
    - Mood baseline throughout day — morning, afternoon, evening
    - "Stress reactivity: how much daily irritations affect you"
    - Energy and cognitive clarity (should be unchanged or improved — not sedated)
    - Any new fatigue, jaundice, right-sided abdominal discomfort, or dark urine (STOP immediately if these appear — liver warning signs)
  notes: "Start with the preparation matching your goal: 400mg standardized extract (10% baicalin) for sleep taken 1–2 hours before bed; 350mg whole herb three times daily for mood and anxiety. Tincture or tea are good starting points if you prefer gradual titration. Unlike valerian, skullcap typically produces no grogginess — the clinical trials specifically measured and found no reduction in energy or cognition. Confirm the product specifies S. lateriflora. Effects are cumulative and subtle — less mental background noise, less reactive to stress, better sleep structure. This is not a pharmacological hammer."

safety:
  generally: caution
  contraindications:
    - "Pre-existing liver disease (current or any history) — ABSOLUTE: documented hepatotoxicity; LiverTox Likelihood Score B"
    - "Warfarin and anticoagulant therapy — ABSOLUTE: S. baicalensis reduced CYP2C9 to 71% of baseline in human trial; warfarin INR may rise dangerously"
    - "Pregnancy and breastfeeding — ABSOLUTE: insufficient safety data; risk-benefit does not support use given documented hepatotoxicity"
    - "Concurrent hepatotoxic medications — additive liver risk"
    - "Phenytoin — CYP2C9 inhibition raises levels; narrow therapeutic index"
    - "Autoimmune conditions (relative) — one documented case of drug-induced autoimmune hepatitis in Sjögren's syndrome patient with severe outcome (dialysis-dependent, listed for liver-kidney transplant)"
  pregnancyNursing: "Contraindicated. Insufficient safety data for both species. Traditional TCM use of S. baicalensis to 'calm the fetus' does not constitute modern safety evidence. Given documented hepatotoxicity potential, risk-benefit does not support use in pregnancy or lactation."
  notes: "Hepatotoxicity is the central safety concern. LiverTox likelihood score B — 'very likely but rare cause of clinically apparent liver injury.' Onset in documented cases: 1 week to 3 months. Injury pattern: hepatocellular, usually self-limiting and reversing within 12 weeks of stopping. However: at least one case required liver transplant listing; one confirmed by positive rechallenge; one involved pulmonary infiltrates requiring oxygen supplementation alongside liver injury. CYP2C9 inhibition to 71% of baseline is clinically significant for warfarin, phenytoin, and NSAIDs (ibuprofen, diclofenac, naproxen, celecoxib). CYP2E1 induced 1.42-fold — use caution with regular acetaminophen or significant alcohol use. Germander (Teucrium canadense) adulteration is a historical problem — germander's own neoclerodane diterpene hepatotoxins may account for some attributed 'skullcap' liver injury. Baseline liver function tests are appropriate before starting; repeat at 4–8 weeks for ongoing use. Stop immediately if jaundice, dark urine, right upper quadrant pain, or unusual fatigue develops."

combinedWith:
  - herb: "valerian"
    why: "Synergistic sleep support — complementary mechanisms, both modulating GABAergic pathways. Classic traditional bedtime combination. Most common pairing in Western herbal practice."
  - herb: "passionflower"
    why: "Complementary nervines for anxiety and nervous restlessness. Both gentle, non-sedating. Good for daytime anxiety without impairment."
  - herb: "lemon balm"
    why: "Nervous tension and daytime stress. Similar gentle profile — the combination is widely used and traditionally validated."
  - herb: "hops"
    why: "Traditional sleep formula. Hops adds a sedative quality that skullcap doesn't provide — useful when you need more than nervous system quieting to fall asleep."

sources:
  - title: "Di Minno et al. 2025 — Efficacy of S. lateriflora Extract for Sleep Management (Nutrients, PMID 40362800)"
    link: "https://pubmed.ncbi.nlm.nih.gov/40362800/"
    type: study
    year: 2025
  - title: "Brock et al. 2014 — American Skullcap Effects on Mood in Healthy Volunteers (Phytother Res, PMID 23878109)"
    link: "https://pubmed.ncbi.nlm.nih.gov/23878109/"
    type: study
    year: 2014
  - title: "Wolfson & Hoffmann 2003 — Efficacy of Scutellaria lateriflora in Healthy Volunteers (Alt Ther, PMID 12652886)"
    link: "https://pubmed.ncbi.nlm.nih.gov/12652886/"
    type: study
    year: 2003
  - title: "Di Minno et al. 2024 — S. lateriflora + Cistus Chewing Gum for Gingivitis (Nutrients, PMID 38542772)"
    link: "https://pubmed.ncbi.nlm.nih.gov/38542772/"
    type: study
    year: 2024
  - title: "Dodd et al. 2025 — S. baicalensis/Crataegus/Magnesium/Chromium in Stressed Adults (J Psychopharmacol, PMID 41194549)"
    link: "https://pubmed.ncbi.nlm.nih.gov/41194549/"
    type: study
    year: 2025
  - title: "Buccato et al. 2024 — In Vitro Cortisol Inhibition, Bioaccessibility, and Bioavailability of S. lateriflora (Molecules, PMID 38338331)"
    link: "https://pubmed.ncbi.nlm.nih.gov/38338331/"
    type: study
    year: 2024
  - title: "Awad et al. 2003 — Phytochemical and Biological Analysis of Skullcap (Phytomedicine, PMID 14692724)"
    link: "https://pubmed.ncbi.nlm.nih.gov/14692724/"
    type: study
    year: 2003
  - title: "Yi et al. 2009 — Effects of Scutellariae Radix on CYP450 Activities in Healthy Volunteers (Basic Clin Pharmacol Toxicol, PMID 19422358)"
    link: "https://pubmed.ncbi.nlm.nih.gov/19422358/"
    type: study
    year: 2009
  - title: "Thakral et al. 2023 — Drug-Induced Autoimmune Hepatitis from Skullcap Supplement (World J Hepatol, PMID 38223420)"
    link: "https://pubmed.ncbi.nlm.nih.gov/38223420/"
    type: study
    year: 2023
  - title: "Soldera 2024 — Insights into Skullcap Herb-Induced Liver Injury (World J Hepatol, PMID 38495279)"
    link: "https://pubmed.ncbi.nlm.nih.gov/38495279/"
    type: study
    year: 2024
  - title: "Liao et al. 2021 — S. baicalensis for Migraine: Taiwan NHIRD + Preclinical Study (Evid Based Complement Alt Med, PMID 33505504)"
    link: "https://pubmed.ncbi.nlm.nih.gov/33505504/"
    type: study
    year: 2021
  - title: "Ma et al. 2024 — S. baicalensis for Major Depressive Disorder: Preclinical Systematic Review (Front Pharmacol, PMID 38572433)"
    link: "https://pubmed.ncbi.nlm.nih.gov/38572433/"
    type: meta-analysis
    year: 2024
  - title: "Brock et al. 2012 — Use of S. lateriflora: Pilot Survey of Herbal Practitioners (J Herbal Med)"
    type: practitioner
    year: 2012
  - title: "LiverTox Database — Skullcap Hepatotoxicity (NIH/NCBI NBK548757)"
    link: "https://www.ncbi.nlm.nih.gov/books/NBK548757/"
    type: other
  - title: "American Herbal Pharmacopoeia — Scutellaria lateriflora Monograph (2009)"
    type: other
  - title: "WHO Monographs on Selected Medicinal Plants, Volume 3 — Radix Scutellariae (S. baicalensis, 2007)"
    type: other

draft: false
---

## What The Evidence Says

Most nervines make you drowsy. Skullcap's distinctive signal is different: it quiets the mental noise without touching your energy or sharpness. That's unusual, and it's what makes the evidence worth paying attention to — even though the evidence base is still thin.

**A necessary species note:** Two very different plants share the name "skullcap." *Scutellaria lateriflora* (American skullcap) is the nervine with sleep and mood evidence. *Scutellaria baicalensis* (Chinese skullcap / Huang Qin) is a TCM anti-inflammatory herb used for heat conditions — not for anxiety or sleep. When you see "skullcap" on a product, it's usually intended as *S. lateriflora*, but the label rarely specifies. This matters for both expected effects and safety.

**Sleep — the strongest evidence:**

If you have mild-to-moderate insomnia, the 2025 trial is directly relevant — it enrolled exactly that population. Over 56 days, people taking 400mg/day of standardized *S. lateriflora* extract (10% baicalin) slept 1.22 more hours per night and rated their sleep quality substantially better on the Pittsburgh Sleep Quality Index (PSQI). The improvement — a 3.5-point PSQI drop — was nearly double what some researchers consider the threshold for noticeable change. The placebo group actually got slightly worse over the same period. [1]

**Mood — consistent, non-sedating signal:**

Two weeks at 1,050mg/day (350mg three times daily) and mood disturbance dropped significantly in 43 healthy volunteers on the Profile of Mood States (POMS) scale — placebo didn't move at all. [2] What's unusual: energy and cognition were unaffected. This is a nervine that improves mood without making you slower. An earlier double-blind crossover (Wolfson & Hoffmann 2003) found similar anxiolytic effects in healthy subjects. [3]

**One honest caveat about anxiety:** When researchers measured anxiety specifically using the Beck Anxiety Inventory (BAI) in the Brock 2014 trial, skullcap didn't beat placebo — the gap wasn't large enough to be convincing. [2] The mood benefit is real; the anxiety benefit is less clear. These aren't the same thing, and it matters for what you expect.

**Combination product signals:**

A 15-day crossover in 35 stressed adults found improved mood, reduced trait anxiety, and better sleep with a *S. baicalensis* combination product — but it also contained hawthorn, magnesium, and chromium. [5] A 3-month gingivitis trial (n=60) found significant gingival bleeding reduction with a *S. lateriflora* and *Cistus* combination chewing gum. [4] Neither result can be credited to skullcap alone, but they add context.

**The honest picture:** The sleep trial is well-designed with a real and meaningful effect. Mood improvement without sedation is consistent across multiple small studies. But the total evidence base is thin — two to three RCTs on *S. lateriflora*, the largest with 66 participants, and no systematic review focused specifically on this species.

## Traditional Use

American herbalists of the 1800s called *S. lateriflora* "one of the most reliable nervines in the materia medica" — not because it knocked people out, but because it calmed excitable, frayed nervous systems. The Eclectic physicians prescribed it for nervous exhaustion, insomnia, involuntary movement disorders, and delirium tremens. The common name "mad dog skullcap" captures the traditional picture: something that settles agitation without dulling the mind.

That tradition is unbroken. A 2012 survey of 62 registered UK and Irish herbalists found them still prescribing *S. lateriflora* for the same core indications — anxiety, stress, muscle tension, digestive complaints driven by nervous tension. [13] Two centuries of consistent nervine use, and the modern trials are now confirming exactly what the Eclectics described: mood improvement without sedation.

**Chinese skullcap (*S. baicalensis* / Huang Qin) — a different plant, a different tradition:**

Chinese practitioners used the root for completely different problems — fever, inflammation, damp-heat conditions. Documented in the *Shennong Bencao Jing* (~2nd century CE), *S. baicalensis* is classified as a bitter, cold, Heat-clearing herb. WHO monograph status (Volume 3, 2007) and European Pharmacopoeia inclusion since 2010. But this is not a nervine by any TCM category — it's an anti-inflammatory herb. The plants share a name and a genus, not a traditional use.

## The Taste

Skullcap is one of the gentler-tasting medicinal herbs — nothing like valerian's pungent funk or ashwagandha's earthy "dirt" flavor. The tincture is mildly bitter and slightly grassy. Most people tolerate it without complaint.

As a tea, it's actually pleasant: a light, hay-like bitterness with green, grassy notes. Steep it covered for 10–15 minutes and the cup is mild enough to drink straight. If you've been avoiding herbal medicine because of taste, skullcap is an easy starting point. Combining it with lemon balm or chamomile rounds things out nicely.

Capsules and standardized extracts bypass taste entirely.

## How To Try It

### First: Confirm the Species

Look for *Scutellaria lateriflora* specified on the label — not just "skullcap." If the label doesn't specify, you don't know what you have. This matters both for expected effects and for authentication (germander adulteration risk is specific to the *S. lateriflora* supply chain — see Quality section below).

### Preparations

**Standardized extract — for sleep:**

400mg/day extract standardized to 10% baicalin, taken once daily, 1–2 hours before bed. This is the preparation and dose from the 2025 trial. If sleep is the primary goal, this is the evidence-based choice. [1]

**Whole herb capsules — for mood:**

350mg three times daily (1,050mg/day total). This is the Brock 2014 protocol that showed significant mood improvement at 2 weeks. [2]

**Tincture — flexible, traditional:**

2–4mL three times daily (1:5 tincture, 45% ethanol). Standard Western herbalism preparation. Allows gradual dose adjustment. Works well in a small glass of water or juice.

**Tea:**

1–2g (roughly 1 teaspoon) of dried aerial parts per cup. Pour just-boiled water, steep 10–15 minutes covered, drink 2–3 cups daily. Combine with lemon balm for daytime calming or valerian for sleep.

### Timing

- **For sleep**: 1–2 hours before bed
- **For mood and daytime anxiety**: Three divided doses through the day

### What to Expect

Skullcap is quieting rather than sedating — unlike valerian or most nervines, it won't fog your brain. The trials measured this directly: energy and cognition were unaffected even when mood improved significantly. [2] You can take it during the day.

The effect is more like a reduction in mental background noise: less rumination, less reactive to daily stressors, easier mental settling. Sleep improvements are structural (better quality and duration) rather than "knocked out."

Mood effects appeared within 2 weeks in the trial. Sleep quality was measured over 56 days. Match your evaluation timeline to the indication.

## What To Track

**Baseline — one week before starting:**
- Sleep: time to fall asleep, number of wakings, quality on waking (1–10)
- Mood: morning and evening rating (1–10)
- Stress reactivity: how often daily events derail you
- Energy and mental clarity

**During trial:**

Track the same markers daily. Compare week 2 and week 8 to baseline. Mood signal should emerge by week 2. Sleep quality improvements may take the full 8 weeks — be patient.

**RED FLAGS — Stop immediately and get blood work:**
- Jaundice (yellowing of skin or whites of eyes)
- Dark, tea-colored urine or pale stools
- Right upper quadrant abdominal pain or unusual tenderness
- Unexplained fatigue or loss of appetite
- Shortness of breath or chest symptoms

These are rare — but they've happened. [10,14] Liver injury from skullcap has onset anywhere from 1 week to 3 months. Early detection and stopping the herb are the intervention. Don't wait to see if symptoms improve.

## Who This Is/Isn't For

### Likely to benefit:

- **Mild-to-moderate insomnia**: This is where the evidence is strongest — a well-designed trial showed real improvement in exactly this population. [1]
- **Stress-related mood disturbance**: Not clinically depressed, but feeling mentally noisy, flat, or reactive. The mood trials enrolled healthy adults and found meaningful improvement at 2 weeks. [2]
- **Wanting non-sedating calming**: Skullcap doesn't impair cognition or energy in trials. Good for daytime use if you need calm without fog.
- **People who do well with gentle nervines**: If lemon balm or passionflower have helped, skullcap adds another tool with more clinical evidence behind it.

What they report: the mental chatter gets quieter. Not knocked out — more like the background hum turns down a few notches. Sleep comes without fighting for it. Daily irritations land differently.

### Unlikely to benefit:

- Those expecting a sedative effect — the trials don't show that
- Clinical anxiety disorders or major depression — no adequate evidence for these
- Healthy people without sleep or mood complaints — trials enrolled symptomatic populations

### Should not use:

**Absolute contraindications:**
- **Pre-existing liver disease** (current or any history): documented hepatotoxicity is the primary safety concern for this herb [9,14]
- **On warfarin or anticoagulants**: In a 13-day human trial of *S. baicalensis* (note: Chinese skullcap, not American), CYP2C9 activity dropped to 71% of baseline — meaning warfarin levels can rise by roughly 30%. [8] Whether *S. lateriflora* has the same effect is unknown, but caution is warranted.
- **Pregnancy or breastfeeding**: Contraindicated — insufficient safety data and documented hepatotoxicity risk

**Use with caution:**
- **On phenytoin, NSAIDs (ibuprofen, diclofenac, naproxen, celecoxib)**: These are processed by the same liver enzyme (CYP2C9) — interaction possible
- **Regular acetaminophen or significant alcohol use**: The same *S. baicalensis* trial [8] found the enzyme that processes acetaminophen and alcohol (CYP2E1) was boosted by about 40%, potentially raising toxicity risk
- **Autoimmune conditions**: One documented case of drug-induced autoimmune hepatitis in a 62-year-old woman with Sjögren's syndrome who used *S. lateriflora* for 1–2 months. She was ultimately listed for liver-kidney transplant. [10]

## Quality Matters

**The germander problem:** Germander (*Teucrium canadense*) has historically been used to adulterate *S. lateriflora* products. Germander contains neo-clerodane diterpenes — including teucrin A — with well-characterized hepatotoxicity, a completely different mechanism from skullcap itself. Some liver injury cases attributed to "skullcap" in the literature may actually be germander toxicity. The American Herbal Pharmacopoeia developed HPLC authentication methods specifically to address this, allowing reliable species verification and detection of germander contamination. [15]

**What this means for you:** Products without authentication certificates carry unknown germander contamination risk. This is not paranoia — it's documented, and the consequences are liver injury.

**What to look for:**
- Species stated on label: *Scutellaria lateriflora* (aerial parts) — not just "skullcap"
- Authentication certificate or COA with species verification and germander testing
- Standardized to 10% baicalin if targeting sleep (matches clinical trial)
- Third-party testing for contaminants and adulterants
- GMP-certified manufacturer

**Monitor your liver:** Baseline liver function tests before starting are reasonable for anyone planning more than a few weeks of use. Repeat at 4–8 weeks for ongoing supplementation. This is precautionary due diligence, not a sign the herb is likely to cause problems — but given documented cases and unknown mechanism, it's appropriate.

**Drug interaction screening:** Before starting, review your full medication list for CYP2C9 substrates. Warfarin is the highest-risk interaction — narrow therapeutic index. The documented CYP2C9 inhibition from *S. baicalensis* [8] is clinically significant and warrants caution with any skullcap species until more data exists.

## The Bottom Line

Skullcap earns its 200-year reputation. The sleep evidence from 2025 is the best we have — not overwhelming, but real and meaningful for the right person. Mood without sedation is a consistent signal across small studies. The total evidence base is thin, but what's there points in the same direction the Eclectic physicians described two centuries ago.

**When it works:** Better sleep architecture, quieter mental baseline, improved mood without grogginess. Effects are subtle and cumulative, not dramatic.

**When it doesn't:** You're either a non-responder or expected too much. This is not a hypnotic, not a primary anxiolytic for clinical anxiety, and not going to produce a noticeable sedation sensation. If sleep or mood don't meaningfully improve in 8 weeks, reassess.

**The non-negotiable caveat:** Liver injury is documented and the mechanism is unknown. The severity range includes routine self-limiting hepatocellular injury and acute liver failure requiring transplant. Rare — but real. Pre-existing liver disease is an absolute contraindication. Warfarin is an absolute contraindication. Get baseline labs and monitor actively.

Use authenticated *S. lateriflora* from a reputable supplier. Screen for drug interactions. Get baseline liver function tests. Give it 8 weeks for sleep, 2 weeks for mood. Stop immediately at any liver warning signs.
